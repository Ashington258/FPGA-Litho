...
### � Phase 7: 修复"输出全零"问题 (进行中)

#### 问题诊断 (已完成 2026-04-05)

- [x] **TODO-7.0**: 问题根因分析 ✓
  **诊断结论**: BRAM版本SOCS计算逻辑是简化占位符，缺少关键步骤
  
  **对比分析**:
  ```
  ❌ BRAM版本错误:
     - kernel_idx = i % 225 (循环取模，逻辑错误)
     - 缺少循环移位步骤
     - 缺少正确Kernel-Mask索引映射
  
  ✓ 正确版本算法 (hls_socs.cpp):
     - krn_idx = (y+Ny)*(2*Nx+1) + (x+Nx) (2D索引)
     - msk_idx = (y+Lyh)*Lx + (x+Lxh) (中央区域提取)
     - 包含循环移位: hls_shift_array_real
  ```

#### 算法移植 (进行中)

- [x] **TODO-7.A.1**: 添加循环移位辅助函数 ✓ (2026-04-05)
  - 文件: `src/hls_litho_system_bram.cpp` (新增函数)
  - 功能: `hls_shift_array_real_bram()` - 将频域中心移到图像中心
  - 实现: 完整移植自 `hls_socs.cpp:21-34`

- [x] **TODO-7.A.2**: 移植完整SOCS算法 ✓ (2026-04-05)
  **修复内容**:
  - 替换 `OP_COMPUTE_SOCS` 分支 (第150-212行)
  - 移植5步完整算法:
    1. 复制mask到本地缓存 ✓
    2. 初始化product和累加器 ✓
    3. Kernel-Mask复数乘法（正确索引映射） ✓
    4. 平方幅度累加 ✓
    5. 循环移位 + 输出截取 ✓
  
  **关键修正**:
  ```cpp
  // 修复前: kernel_idx = i % 225 ❌
  // 修复后: 正确2D索引映射 ✓
